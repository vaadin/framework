...
public class HtmlBundleHandler implements Serializable {
    ...
    private static String getHtmlBundleName(VaadinSession session) {
        ...
        String defaultBundleFilename = BootstrapHandler
                .resolveFrontendUrl(session).replace("vaadin://", "/VAADIN/")
                + "bundle.html";
        VaadinServlet servlet = ((VaadinServletService) vaadinService)
                .getServlet();
        try {
            URL autoDetected = servlet.findResourceURL(defaultBundleFilename);
            if (autoDetected != null) {
                return defaultBundleFilename;
            }
        } catch (IOException e) {
            getLogger().log(Level.FINE,
                    "Error finding default bundle " + defaultBundleFilename, e);
        }
