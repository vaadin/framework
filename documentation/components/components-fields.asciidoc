...
more sophisticated validation diagnostics (ex. locale-specific), there is
[methodname]#withValidator# method, which uses boolean function and [classname]#ErrorMessageProvider#.
[classname]#ErrorMessageProvider# can compose diagnostic messages based on locale
and source component value, provided with [classname]#ValueContext#.
