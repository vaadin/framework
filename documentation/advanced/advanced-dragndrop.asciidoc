...
It is possible to drag and drop the rows of a Grid component allowing reordering of rows, dragging rows between different Grids, dragging rows outside of a Grid or dropping elements onto rows.

=== Grid Drag Source

A Grid component's rows can be made draggable by applying [classname]#GridDragSource# extension to the component.

You can define the allowed effects and provide a drag data generator.

[source,java]
----
Grid<Bean> draggableGrid = new Grid<>();
// ...
GridDragSource<Bean> dragSource = new GridDragSource<>(dragSourceComponent);
...
dragSource.setDragDataGenerator(bean -> {
    ...
    ret.put("beanId", bean.getId());
    ret.put("beanValue", bean.getValue());
    return ret;
});
----

The _drag data generator_ defines what data should be transferred when a row is dragged and dropped. The generator is executed for every inserted item and returns a `JsonObject` containing the data to be transferred for that item.

[NOTE]
====
Note that setting the data transfer text is not supported.
====
...
dragSource addGridDragEndListener(event -> {
    ...
    if (event.getDropEffect() == DropEffect.MOVE) {
        ((ListDataProvider<Person>) source.getDataProvider()).getItems()
                .removeAll(event.getDraggedItems() );
...
To make a Grid component's rows accept a drop event, apply the [classname]#GridDropTarget# extension to the component. When creating the extension you need to define the drop mode.
...
GridDropTarget<Bean> dropTarget = new GridDropTarget<>(grid, DropMode.BETWEEN);
dropTarget.setDropEffect(DropEffect.MOVE);
----
...
The [classname]#GridDropEvent# is fired when an element is dropped onto a row or between rows.

[source,java]
----
dropTarget.addGridDropListener(event -> {
    // Accepting dragged items from another Grid
    event.getDragSourceExtension().ifPresent(source -> {
        if (source instanceof GridDragSource) {
            // Get the target Grid's items
            ListDataProvider<Bean> dataProvider = (ListDataProvider<Bean>)
                    dropEvent.getComponent().getDataProvider();
            List<Bean> items = (List<Bean>) dataProvider.getItems();

            // Calculate the target row's index
            int index = items.indexOf(dropEvent.getDropTargetRow())
                    + dropEvent.getDropLocation() == DropLocation.BELOW ? 1 : 0;
