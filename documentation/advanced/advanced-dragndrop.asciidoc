...
The drag source element, additional to it's primary style name, have a style name with the `-dragsource` suffix. For example, a Label component would have the style name `v-label-dragsource`.
...
When dragging data over a drop target, a style name is applied to indicate that the element accepts drops. This style name is the primary style name with `-drag-center` suffix, e.g. `v-label-drag-center`.
...
A drag source Grid's rows have the `v-grid-row-draggable` style name applied to indicate that the rows are draggable.
