...
If the old URI fragment based navigation should be used, the [classname]#PushStateNavigation# annotation can be removed from the [classname]#UI#. This will use the default navigation.
...
By default the URLs managed through the [classname]#Navigator# have an URI fragment
...
If the [classname]#PushStateNavigation# is present on the [classname]#UI# the HTML5
History API is used. When using the PushState, the identifier is separated from the
root path by a [literal]#++/++# like a real URL.

In addition to the View identifier, URI can contain additional parameters to be 
passed for views. The parameters are the part of the URI after the longest matched view identifier, separated by [literal]#++/++#. These parameters in together with the identifier form the __navigation state__.
