...
// set other data to transfer (in this case JSON data)
dragSource.setDataTransferData("application/json", "{\"message\":\"hello\"}");
----
...
dropTarget.addDropListener(event -> {
    ...
    if (dragSource.isPresent() && dragSource.get() instanceof Label) {
        ...
        // get possible transfer data of other type
        String jsonData = event.getDataTransferData("application/json");
        if (jsonData != null) {
            Notification.show("Also has JSON: " + jsonData);
        }
...
The _drag data generator_ defines what data should be transferred with the given type when a row is dragged and dropped.
Each generator is executed for every inserted item and returns a `String` containing the data to be transferred for that item.
When multiple rows are dragged, the generated data is combined into one String separated by new line characters (`"\n"`).
The data is then transferred using the HTML5 DataTransfer's data parameter of the given type.
By default, the data of type `"text"` will contain the content of each column separated by a tabulator character (`"\t"`). You can override this behaviour by setting a custom generator for the `"text"` type.
