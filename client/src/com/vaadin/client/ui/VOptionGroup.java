...
public class VOptionGroup extends VOptionGroupBase implements FocusHandler,
    ...
    public void buildOptions(UIDL uidl) {
        ...
        for (final Iterator<?> it = uidl.getChildIterator(); it.hasNext();) {
            ...
            boolean opEnabled = !opUidl
                    .getBooleanAttribute(OptionGroupConstants.ATTRIBUTE_OPTION_DISABLED);
            boolean enabled = opEnabled && !isReadonly() && isEnabled();
            op.setEnabled(enabled);
            optionsEnabled.add(opEnabled);
    ...
    public void setEnabled(boolean enabled) {
        ...
        int i = 0;
        // sets options enabled according according to the widget's enabled,
        // isReadonly and each options own enabled
        for (Iterator<Widget> it = panel.iterator(); it.hasNext();) {
