...
public interface IDefaultConverter extends Serializable {
    ...
    /**
     * Converts a {@link Binder.BindingBuilder} to a given class for an entity
     * property
     *
     * @param builder
     *            the field builder
     * @param propertyType
     *            the entity property class type
     * @return the converted binder builder
     */
    default Binder.BindingBuilder build(Binder.BindingBuilder builder,
            Class<?> fieldType, Class<?> propertyType) {
        if (isSupported(fieldType, propertyType)) {
            if (fieldType.equals(String.class)) {
                if (propertyType == Integer.class) {
                    ...
                    builder = builder.withNullRepresentation("").withConverter(
                            new StringToDoubleConverter("Must be a double!"));
                } else if (propertyType == Long.class) {
                    // Convert java.lang.String to java.lang.Long
                    builder = builder.withNullRepresentation("").withConverter(
                            new StringToLongConverter("Must be a long!"));
                } else if (propertyType == Float.class) {
                    // Convert java.lang.String to java.lang.Float
                    builder = builder.withNullRepresentation("").withConverter(
                            new StringToFloatConverter("Must be a float!"));
                }
    ...
    default Map<Class<?>, Set<Class<?>>> getSupportedConverts() {
        ...
        map.put(String.class, new HashSet<>(Arrays.asList(String.class,
     ...
     *
     * @param fieldClass
     *            class of the field (String for
     *            {@link com.vaadin.ui.TextField})
     * @param propertyType
    ...
    default boolean isSupported(Class<?> fieldClass, Class<?> propertyType) {
        ...
        return supported != null && supported.size() > 0
                && supported.contains(propertyType);
    }
}
