...
[WARNING]
The [classname]#TwinColSelect# does not provide any lazy load mechanism.
Hence it will slow down significantly if used with large itemsets.
This lazy load feature could be implemented creating a server-side custom component that
uses two single column Grids instead.
