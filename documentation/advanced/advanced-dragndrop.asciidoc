...
It is possible to drag and drop the rows of a Grid component. This allows reordering of rows, dragging rows between different Grids, dragging rows outside of a Grid or dropping elements onto rows.
...
dragSource.setDragDataGenerator(item -> {
    JsonObject ret = Json.createObject();
    ret.put("beanId", item.getId());
    ret.put("beanValue", item.getValue());
    return ret;
});
----
...
[NOTE]
====
Note that calling the `setDataTransferText(String data)` method is not supported.
====
