...
public class BinderValueChangeTest
    ...
    private AtomicReference<BinderChangeEvent<Person, ?>> event;
    ...
    public void unboundField_noEvents() {
        binder.addBinderChangeListener(this::statusChanged);
    ...
    private void verifyEvent(HasValue<?> field) {
        BinderChangeEvent<Person, ?> statusChangeEvent = event.get();
        Assert.assertNotNull(statusChangeEvent);
        Assert.assertEquals(binder, statusChangeEvent.getBinder());
        Assert.assertEquals(field, statusChangeEvent.getSource());
        Assert.assertEquals(field, statusChangeEvent.getComponent());
        Assert.assertFalse(statusChangeEvent.isUserOriginated());
    }

    private void statusChanged(BinderChangeEvent<Person, ?> evt) {
        Assert.assertNull(event.get());
        event.set(evt);
    }
}
