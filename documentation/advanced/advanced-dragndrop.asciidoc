...
The drag image can be customized using the `dnd-poly-drag-image` class name. The following styling can be used to make the drag image "snap back" when the user has aborted the drag:
