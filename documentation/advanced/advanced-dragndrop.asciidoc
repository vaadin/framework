...
You can handle dropped files with the `FileDropHandler` that you add as the second constructor parameter. The `FileDropEvent`, received by the handler, contains information about the dropped files such as file name, file size and mime type.
Here you can decide if you would like to upload each of the dropped files.
