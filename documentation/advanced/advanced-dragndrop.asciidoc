...
The _drop location_ property in the [classname]#GridDropEvent# specifies the dropped location in relative to grid row the drop happened on. Possible options are `ON_TOP`, `ABOVE` and `BELOW`, when the drop happened on top of a row. If the grid is empty, or if there was empty space after the last row in grid and the [classname]#DropMode.ON_TOP# was used, then the drop location `EMPTY` will be used. Then the [methodname]#getDropTargetRow# will also return an empty optional.
