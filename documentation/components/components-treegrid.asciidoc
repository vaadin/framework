...
It can be set with [methodname]#setItemCollapseAllowedProvider# method, that takes a [classname]#SerializablePredicate#.

Avoid doing any heavy operations in the method, since it is called for each item when it's being sent to the client.
