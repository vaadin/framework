...
It is possible to transfer any Object as server side data to the drop target if both the drag source and drop target are placed in the same UI.
...
dropTarget.addDropListener(event -> {
    ...
    if (dragSource.isPresent() && dragSource.get() instanceof Label) {
        ...
        // handle possible server side drag data
