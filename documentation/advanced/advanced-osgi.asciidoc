...
When using push with any container, it is important that your version of Jetty/Tomcat is compatible with Vaadin's
port of Atmosphere.  The server log will report errors and the browser console will display a fallback warning
if there are incompatibilities.

= Karaf Features
Using a Karaf feature is the easiest way to install and maintain the required bundles.

== Adding The Feature
You can add the feature file using the _repo-add_ command, or by placing it in your Karaf _deploy directory_

[source]
----
> feature:repo-add file:vaadin-feature.xml
----
or
[source]
----
<path-to-karaf>/karaf/deploy/vaadin-feature.xml
----

== Installing The Feature
After adding the feature, use the following command to install the Vaadin OSGi framework.

[source]
----
> feature:install vaadin-osgi-core 
----
or
[source]
----
> feature:install vaadin-osgi-push 
----

== file: vaadin-feature.xml
Here is a preliminary Karaf feature:

[source, xml]
----
<features name="vaadin-osgi" version="8.1.0" xmlns="http://karaf.apache.org/xmlns/features/v1.0.0">
