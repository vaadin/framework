...
public class SingleFilterSearcherTest {
    private SingleFilterSearcher<String, SerializablePredicate<String>> searcher = new SingleFilterSearcher<>(
            DataProvider.create("one", "two", "three"));

    @Test
    public void basics() {

        assertItems(searcher, "one", "two", "three");

        searcher.searchBy("two"::equals);
        assertItems(searcher, "two");
