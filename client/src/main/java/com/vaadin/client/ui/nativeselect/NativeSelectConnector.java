...
public class NativeSelectConnector
    ...
    private void onEmptySelectionCaptionChange() {
        updateEmptySelectionItem();
    }

    @OnStateChange("emptySelectionAllowed")
    private void onEmptySelectionAllowed() {
        updateEmptySelectionItem();
    }
    ...
    private void updateEmptySelectionItem() {
        ListBox listBox = getWidget().getListBox();
        boolean isEmptyItem = listBox.getItemCount() > 0
                && listBox.getValue(0).isEmpty();
        if (isEmptyItem && getState().emptySelectionAllowed) {
            listBox.setItemText(0, getState().emptySelectionCaption);
        } else if (isEmptyItem && !getState().emptySelectionAllowed) {
