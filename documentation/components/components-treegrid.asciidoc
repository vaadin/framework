...
[classname]#TreeGrid# is used by binding it to a hierarchical data provider. The data provider can be based on in-memory or back end data. For in-memory data, the [classname]#InMemoryHierarchicalDataProvider# can be used, and for loading data from a back end, you need to implement three methods from the [interface]#HierarchicalDataProvider# interface. Usage of both data providers is described in
...
collection, that the data provider uses to populate the [classnam]#TreeGrid#.

The [methodname]#setItems# method in [classname]#TreeGrid# can be used to set the root level items. Internally
a [classname]#InMemoryHierarchicalDataProvider# with [classname]#HierarchyData# is used. If at any time you want to modify the in-memory data in the grid, you may do it as follows
...
// after adding / removing data, data provider needs to refresh
dataProvider.refreshAll();
----

Note that for any updates, you always need to call the [methodname]#refreshAll# method in the data provider you are using. The [methodname]#refreshItem# method can be used when just the data for a single item is updated.
