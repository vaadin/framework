...
public class HtmlImportUITest extends MultiBrowserTest {
    ...
    public void importsLoadedAfterJs() {
        ...
        Assert.assertEquals("Some log messages are missing", 3,
                messages.size());
        ...
        Integer labelJsIndex = jsIndexes.get("label.js");
        Integer labelHtmlIndex = htmlImportIndexes.get("label.html");
        Assert.assertNotNull(labelJsIndex);
        Assert.assertNotNull(labelHtmlIndex);
        Assert.assertTrue(
                "label.js script tag should have been before label.html import tag",
                labelJsIndex < labelHtmlIndex);
    }
}
